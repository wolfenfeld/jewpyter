...
*"What do you mean, reuse it?"* asked Devorah. *"The autoencoder was trained on reconstruction. We never told it what the digits are. How does it help with something that requires knowing?"*

This is the right question.

The encoder was never given a label. It never saw the word "seven." But in order to compress a 7 well enough to reconstruct it, it had to learn that a 7 looks a certain way — angular, with a horizontal stroke — and that this is different from a 1 (vertical, no stroke) or a 4 (two strokes, a corner). The label was never provided, but the *meaning of the difference* was forced on it by the reconstruction task.

That meaning lives in the latent code. And we can exploit it.

**The setup:** freeze the encoder (do not update its weights). Attach a small classifier — just one linear layer — on top of the latent representation. Train only the classifier on labeled examples. The encoder does the heavy lifting; the classifier just learns to sort the encoder's output into buckets.

One important caveat first: **2 latent dimensions is too small for useful transfer**. Two numbers can barely distinguish ten digit classes — our 2D autoencoder was designed for visualization, not for carrying rich features. For transfer learning you would use a larger bottleneck, typically 32–256 dimensions depending on the complexity of the data. The reconstruction quality stays high; you just get richer representations to hand downstream.

Here we train a separate autoencoder with `latent_dim=32` for the transfer experiment:
...
class Autoencoder(nn.Module):
    def __init__(self, latent_dim=32):   # 32, not 2
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Flatten(),
            nn.Linear(784, 256), nn.ReLU(),
...
# Train on unlabeled data (reconstruction only)
transform = transforms.ToTensor()
...
for epoch in range(20):
    for images, _ in loader:          # labels are ignored during pretraining
        recon, _ = model(images)
        loss = criterion(recon, images.view(-1, 784))
        optimizer.zero_grad(); loss.backward(); optimizer.step()
```

Now freeze the encoder and train a linear classifier on top with a small number of labeled examples:
...
# Encode the full dataset — no labels needed here
model.eval()
def encode(dataset):
    ...
    with torch.no_grad():
        for imgs, labels in loader:
            zs.append(model.encoder(imgs).numpy())
            ys.append(labels.numpy())
    return np.concatenate(zs), np.concatenate(ys)

Z_train, y_train = encode(train_data)
Z_test,  y_test  = encode(datasets.MNIST('.', train=False, transform=transform))

# Train a logistic regression on only 500 labeled examples
n = 500
scaler = StandardScaler()
clf = LogisticRegression(max_iter=500)
clf.fit(scaler.fit_transform(Z_train[:n]), y_train[:n])
print(f"Accuracy: {clf.score(scaler.transform(Z_test), y_test):.1%}")
```

Compare to a baseline: logistic regression on raw pixels (784 features) with the same number of labeled examples — no pretrained encoder. Same data, same task, same classifier. The real advantage shows when labeled data is scarce:

<iframe src="/assets/charts/autoencoder-transfer.html" style="width:100%;height:480px;border:none;"></iframe>

With 100 labeled examples, the 32D encoder achieves around **70% accuracy** while raw pixels manage around **68%** — already ahead, from a model that never saw a single label during training. At 50 examples the gap widens: **68% vs 63%**. As you add more labeled data the gap narrows, because eventually the raw-pixel model has enough signal to figure out the structure itself. But the encoder stays ahead throughout.

The gap is largest at the low end precisely because the encoder's representations are already organized: the encoder knows that different-looking digits should be far apart in latent space; the classifier just needs to find the boundary. A raw-pixel model has to discover that organization from scratch using only the few labels available.

This is the core promise of transfer learning: **representations learned on abundant unlabeled data can compensate for scarcity of labeled data.** In the real world, unlabeled data is cheap (photographs, text, audio) and labeled data is expensive (human annotation, clinical trials, expert review). An encoder trained on reconstruction gives you useful structure for free.

The same principle underlies BERT, GPT, and every large pretrained model: first train on a cheap self-supervised task (reconstruct masked words, predict the next token), then fine-tune on the expensive labeled task with very little data. The autoencoder is the toy version of that idea.

*"So the reconstruction was never the point,"* said Devorah.

*"The reconstruction was the excuse,"* said Mathityahu. *"The representations were the point."*
